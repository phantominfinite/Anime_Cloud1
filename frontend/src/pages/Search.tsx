...
import { searchAnime, type JikanAnime, type SearchFilters } from '../services/api';
...
const Search: React.FC = () => {
  ...
  const [filters, setFilters] = useState<SearchFilters>({ status: '', type: '' });
  const [scrollTop, setScrollTop] = useState(0);
  const navigate = useNavigate();

  const normalizedQuery = useMemo(() => query.trim(), [query]);
  const resultsQuery = useQuery({
    queryKey: ['search', normalizedQuery, filters.status, filters.type],
    queryFn: () => searchAnime(normalizedQuery, filters),
    enabled: normalizedQuery.length >= 3,
  });

  const results: JikanAnime[] = resultsQuery.data ?? [];
  ...
  return (
    <div className="pb-20 pt-4 animate-slide-up" onScroll={(e) => setScrollTop((e.target as HTMLDivElement).scrollTop)}>
      <div className="glass sticky top-24 z-20 mb-6 flex items-center rounded-2xl p-2 shadow-xl">
        ...
        <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} className="w-full border-none bg-transparent px-2 py-3 text-lg font-medium text-white placeholder-gray-500 focus:outline-none" placeholder="جستجوی پیشرفته انیمه..." autoFocus />
        <button onClick={() => setShowFilters(!showFilters)} className={`rounded-xl p-3 transition ${showFilters ? 'bg-primary text-white' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`}><SlidersHorizontal size={20} /></button>
      </div>
      ...
      {resultsQuery.isLoading ? <div className="py-20 text-center">در حال جستجو...</div> : results.length > 0 ? (
        <VirtualGrid items={results} scrollTop={scrollTop} viewportHeight={800} rowHeight={320} renderItem={(anime) => (
